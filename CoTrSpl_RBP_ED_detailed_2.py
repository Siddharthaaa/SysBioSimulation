...
if figure == "Fig 3B1":
    ...
    vpol_profile_vpols = np.logspace(0,2.5,50)
    rbp_br= 5
    k2 = 0.5*factor
    rbp_inh = 0.99
...
if figure == "Fig 3B2":
    ...
    vpol_profile_rbp_pos = 300
    rbp_inh = 0.999
    vpol_profile_vpols = np.logspace(1,3.5,50)
    rbp_br= 5
    k2 = 2*factor
    sim_series_vpol_ext = False
...
if figure == "Fig 3B3":
    sim_series_vpol = True
    vpol_profile_rbp_pos = 300
    vpol_profile_vpols = np.logspace(1,3,50)
    rbp_br= 5
    k2 = 2*factor
    rbp_inh = 0.99
...
te6 = bs.TimeEvent("gene_len/vpol", "ret_r = ret_r_t", name="retention possible")
#
s.add_timeEvent(te1)
s.add_timeEvent(te2)
s.add_timeEvent(te3)
...
#        psi = s.get_psi_mean(ignore_fraction = 0.4)
    ...
    if (sim_series_vpol_ext):
        ...
        ax.plot(vpols, rbp_reacts, lw=1.5, ls="--", label="% of spl. after RBP bind.")
