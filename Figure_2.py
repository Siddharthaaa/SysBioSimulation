...
fig_all, axs = plt.subplots(len(models),1, figsize=figsize,
                        squeeze=False,
                        sharex=True)
...
#    axs[1].set_xscale("log")
    ...
    axs[m_i,0].plot(vpols, psis_td, lw = 4, c = "green", label= "TD model")
    ...
    for i, (k, v)  in enumerate(model["hs"].items()):
        axs[m_i,0].axhline(v, ls=":", lw=1,c="C"+str(i), label = k)
