...
if __name__ == "__main__":
    ...
    #            psi = incl_ss/(incl_ss+skip_ss) 
            ...
            name="Ka=%2.2f,n1 =%2.2f" % (Ka1, n1)
            s = SimParam(name, 5000, 10000,
                         params = {"k_on": k_on, "k_off": k_off, "v_syn": k_syn, "s1": s1, "s1_t":s1_t, "Ka1":Ka1, "Ka2":Ka2,
                                   "s2": s2, "s3": s3, "d0": d0, "d1": d1, "d2": d2, "d3": d3, "n1":n1, "n2":n2},
...
#            s.add_reaction("s1*pre_RNA + 5* (Incl > 0)* 1/(1+(Ka1/Incl)**n1)", {"pre_RNA":-1, "Incl":1})
...
#    flattened_sims[indices[0]].get_bimodality()
    ...
    plot_3d_heat(sims, axes=("Ka1","n1"), ignore_extremes=False, recognize_threshold = intensity_th)
    plot_heatmap(sims, axes=("Ka1","n1"),  ignore_extremes=False, recognize_threshold = intensity_th)
